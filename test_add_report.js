...
async function testAddReport() {
  try {
    console.log('بدء اختبار إضافة تقرير مع الملفات...');
    ...
    formData.append('title', 'تقرير تجريبي للاختبار');
    formData.append('summary', 'ملخص تجريبي للتقرير');
    formData.append('content', '<p>محتوى تجريبي للتقرير</p>');
    formData.append('status', 'draft');
    
    // إضافة ملف PDF
    const pdfFile = fs.createReadStream(pdfPath);
    formData.append('pdfFile', pdfFile, 'test_report.pdf');
    
    // إضافة صورة مصغرة
    const imageFile = fs.createReadStream(imagePath);
    formData.append('thumbnail', imageFile, 'test_thumbnail.png');
